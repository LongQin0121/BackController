...
class FlightOptimizer:
    ...
    def __init__(self, command_manager):
        ...
        print(f"🔄 灵活进近: {len(self.flexible_zones)} 条航线支持ETA窗口优化")
        
        if 'MP' in self.waypoints:
            mp = self.waypoints['MP']
            print(f"🎯 MP坐标: {mp['lat']:.4f}, {mp['lon']:.4f}")

    def _calculate_eta_range(self, state):
        """计算最早和最晚ETA"""
        ...
        if route_name not in self.flexible_zones:
            # 非灵活进近，使用直线距离
            distance = state['distance_to_mp']
            eta = distance / ground_speed * 60
            return {'earliest_eta': eta, 'latest_eta': eta, 'time_window': 0}
        
        # 灵活进近航线
        zone = self.flexible_zones[route_name]
        start_point = zone['start']  # IR15, L3, L17
        end_point = zone['end']      # IL17, R21, R21
        
        # 最早ETA：当前位置 → 起始点 → 直飞MP
        if start_point in self.waypoints:
            start_pos = self.waypoints[start_point]
            distance_to_start = calculate_distance(lat, lon, start_pos['lat'], start_pos['lon'])
            distance_start_to_mp = calculate_distance(start_pos['lat'], start_pos['lon'], 
                                                    mp_pos['lat'], mp_pos['lon'])
            earliest_distance = distance_to_start + distance_start_to_mp
            earliest_eta = earliest_distance / ground_speed * 60
        else:
            earliest_eta = state['distance_to_mp'] / ground_speed * 60
        
        # 最晚ETA：当前位置 → 完整弧线 → 终点 → MP
        if end_point in self.waypoints:
            end_pos = self.waypoints[end_point]
            # 简化：假设弧线距离是起始到终点距离的1.5倍（临时）
            distance_to_end = calculate_distance(lat, lon, end_pos['lat'], end_pos['lon']) * 1.5
            distance_end_to_mp = calculate_distance(end_pos['lat'], end_pos['lon'],
                                                  mp_pos['lat'], mp_pos['lon'])
            latest_distance = distance_to_end + distance_end_to_mp
            latest_eta = latest_distance / ground_speed * 60
        else:
            latest_eta = earliest_eta
        
        time_window = latest_eta - earliest_eta  # 可优化的时间窗口
    ...
    def _analyze_aircraft(self, aircraft):
        ...
        # 计算ETA范围
        eta_info = self._calculate_eta_range(state)
        state.update(eta_info)
    ...
    def _display_aircraft_status(self, arrival_aircraft):
        ...
        for state in arrival_aircraft:
            ...
            # 显示ETA范围
            earliest_eta = state['earliest_eta']
            ...
            print(f"     地速: {ground_speed:.0f}kt | 距MP: {distance_to_mp:.1f}nm")
            
            if time_window > 0:
                print(f"     ETA窗口: {earliest_eta:.1f}min (最早) ~ {latest_eta:.1f}min (最晚) | 可优化: {time_window:.1f}min")
            else:
                print(f"     ETA: {earliest_eta:.1f}min (固定航线)")
    ...
    def _optimize_and_command(self, arrival_aircraft):
        ...
        # 按最早ETA排序（最小化延误）
        sorted_aircraft = sorted(arrival_aircraft, key=lambda x: x['earliest_eta'])
        
        print("📋 按最早ETA排序的进港序列:")
        for i, state in enumerate(sorted_aircraft):
            earliest_eta = state['earliest_eta']
            time_window = state['time_window']
            print(f"  {i+1}. {state['callsign']} - 最早ETA: {earliest_eta:.1f}min (窗口: {time_window:.1f}min)")
...
if __name__ == '__main__':
    ...
    print("🔧 约束条件: FL020@180kt过MP, 间隔≥5nm, 下降率≤2000fpm, ETA窗口优化")
